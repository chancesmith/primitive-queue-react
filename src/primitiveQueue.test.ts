...
describe("queue", () => {
  ...
  it("should start empty", () => {
    const queue = createPrimitiveQueue<string>(handler);
  ...
  it("should see into any item in queue", () => {
    const queue = createPrimitiveQueue<string>(handler);
    expect(queue.peekAt(download1)).toBeUndefined();
    queue.enqueue(download1);
    queue.enqueue(download2);
    expect(queue.peekAt(download1)).toBe(download1);
  });
  it("should drop item from queue by ID", () => {
    const queue = createPrimitiveQueue<string>(handler);
    expect(queue.drop(download1)).toBe(false);
    queue.enqueue(download1);
    expect(queue.length()).toBe(1);
    expect(queue.drop(download1)).toBe(true);
    expect(queue.length()).toBe(0);
  });
